...
func patchStruct(patch, val cue.Value, opts *flags.RootPflagpole) (cue.Value, bool) {
	ctx := val.Context()
...
func patchList(patch, val cue.Value, opts *flags.RootPflagpole) (cue.Value, bool) {
	ctx := val.Context()
	oi, _ := patch.List()
	ni, _ := val.List()
