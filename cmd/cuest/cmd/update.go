...
var UpdateLong = `Print the build version for cuest`
...
Updates available. v%s -> %s
  ...
  run 'cuest update' to get the latest.
...
func CheckUpdate(manual bool) (ver ProgramVersion, err error) {
	if !manual && os.Getenv("CUEST_UPDATES_DISABLED") != "" {
		return
	}
	UpdateStarted = true
	cur := ProgramVersion{Version: "v" + verinfo.Version}

	checkURL := "https://api.github.com/repos/hofstadter-io/cuetils/releases/latest"
	if UpdateVersionFlag != "" {
		checkURL = "https://api.github.com/repos/hofstadter-io/cuetils/releases/tags/" + UpdateVersionFlag
		manual = true
	}
...
func WaitPrintUpdateAvailable() {
	if os.Getenv("CUEST_UPDATES_DISABLED") != "" {
		return
	}
