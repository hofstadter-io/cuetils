...
import (
	...
	"github.com/hofstadter-io/cuetils/cmd/cuetils/verinfo"
)
...
Updates available. v%s -> %s
  ...
  run 'cuetils update' to get the latest.
...
func CheckUpdate(manual bool) (ver ProgramVersion, err error) {
	if !manual && os.Getenv("CUETILS_UPDATES_DISABLED") != "" {
		return
	}
	UpdateStarted = true
	cur := ProgramVersion{Version: "v" + verinfo.Version}
...
func WaitPrintUpdateAvailable() {
	if os.Getenv("CUETILS_UPDATES_DISABLED") != "" {
		return
	}
...
func InstallUpdate() (err error) {
	fmt.Printf("Installing cuetils@%s\n", UpdateAvailable.Version)
